...
from .group import KNNGroup, QueryAndGroup, create_grouper
...
class SubsampleGroup(nn.Module):
    ...
    def forward(self, p, x=None):
        ...
        if x is not None:
            ...
            grouped_p, grouped_x = self.grouper(center_p, p, x)
            return grouped_p, center_p, grouped_x, center_x
        else:
...
class PointPatchEmbed(nn.Module):
    ...
    def forward(self, p, x=None):
        ...
        # query neighbors.
        dp, grouped_x = self.grouper(center_p, p, x)
        ...
        if self.feature_type == 'dp':
            ...
            neighborhood_x = torch.cat([dp, grouped_x], dim=1)
        elif self.feature_type == 'dp_df':
            center_x = torch.gather(x, 2, idx.unsqueeze(1).expand(-1, x.shape[1], -1)) 
            neighborhood_x = torch.cat([dp, grouped_x - center_x.unsqueeze(-1)], dim=1)
        elif self.feature_type == 'df':
            center_x = torch.gather(x, 2, idx.unsqueeze(1).expand(-1, x.shape[1], -1)) 
            neighborhood_x = grouped_x - center_x.unsqueeze(-1)
        neighborhood_x = self.conv1(neighborhood_x)

        neighborhood_x = torch.cat(
            [self.pool(neighborhood_x).expand(-1, -1, -1, self.group_size),
             neighborhood_x],
            dim=1)
        out_x = self.pool(self.conv2(neighborhood_x)).squeeze(-1)
        return [p, center_p], [x, out_x]